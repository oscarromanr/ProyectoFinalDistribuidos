 ...
 * @author Fernando
 */
public class frmGraficaDatosTemperatura extends javax.swing.JFrame implements iEventListener{
    
    private JPanel panel;
    DefaultXYDataset dataset;
    
    public frmGraficaDatosTemperatura() {
        dataset = new DefaultXYDataset();
        setTitle("Grafica de temperatura");
        setSize(750,500);
    ...
    private void anadirDatos(String temperatura, String idSensor)
    {
        ...
        List<Alarma> alarmas = CtrlAlarmas.getInstance().consultarPorTipo("Temperatura");
        int u = 0;
        while(u < alarmas.size())
        {
            if(alarmas.get(u).getLimite() <= temperaturaDouble)
            {
                CtrlAlarmas.getInstance().enviarCorreo("rob8meza@hotmail.com", "Alarma activada", "La alarma con el id: "+alarmas.get(u).getId()+" ha sonado, "+alarmas.get(u).getTipo()+": "+temperaturaDouble+", Sensor: "+idSensor);
                System.out.println("La alarma con el id: "+alarmas.get(u).getId()+" ha sonado, "+alarmas.get(u).getTipo()+": "+temperaturaDouble+", Sensor: "+idSensor);
            }
            u++;
        }
        int i = 0;
        while(i < dataset.getSeriesCount())
        {
            ...
            if(nombreSerie.equals(idSensor))
            {
                ...
                yDataAgregar[j] = temperaturaDouble;
        ...
        xDataAgregar[0] = 1.0;
        yDataAgregar[0] = temperaturaDouble;
        double[][] dataSerieAgregar = {xDataAgregar, yDataAgregar};
    ...
    private void init() {
        ...
        JFreeChart chartTemperatura = ChartFactory.createXYLineChart(
            "Medidas de temperatura", // Título del gráfico
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(frmGraficaDatosTemperatura.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new frmGraficaDatosTemperatura().setVisible(true);
            }
        });
    }
    ...
    public void update(String mensaje) {
        ...
        if (indiceInicioPalabraTemperatura != -1) {
            int indiceFinPalabraTemperatura = indiceInicioPalabraTemperatura + palabraBuscadaTemperatura.length();
            String temperaturaBinario = mensaje.substring(indiceFinPalabraTemperatura, indiceInicioPalabraGrados);
            temperatura = ConversorBinario.getInstance().revertirFormatoBinario(temperaturaBinario);
        } else {
            System.out.println("La palabra '" + palabraBuscadaTemperatura + "' no se encuentra en la cadena.");
        }
        
        anadirDatos(temperatura, idSensor);
